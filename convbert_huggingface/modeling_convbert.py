...
def load_tf_weights_in_convbert(model, tf_checkpoint_path):
    ...
    model.embeddings.position_embeddings.weight.data = mydict[
        f"electra/embeddings/position_embeddings"]
    model.embeddings.token_type_embeddings.weight.data = mydict[
        f"electra/embeddings/token_type_embeddings"]
    model.embeddings.LayerNorm.weight.data = mydict[
    ...
    for i in range(12):
        ...
        getattr(model.encoder.layer,
                i).attention.self.query.weight.data = mydict[
                    f"electra/encoder/layer_{i}/attention/self/query/kernel"].T
        getattr(model.encoder.layer,
                i).attention.self.query.bias.data = mydict[
                    f"electra/encoder/layer_{i}/attention/self/query/bias"]
        getattr(model.encoder.layer,
                i).attention.self.key.weight.data = mydict[
                    f"electra/encoder/layer_{i}/attention/self/key/kernel"].T
        getattr(model.encoder.layer, i).attention.self.key.bias.data = mydict[
            f"electra/encoder/layer_{i}/attention/self/key/bias"]
        getattr(model.encoder.layer,
                i).attention.self.value.weight.data = mydict[
                    f"electra/encoder/layer_{i}/attention/self/value/kernel"].T
        getattr(model.encoder.layer,
                i).attention.self.value.bias.data = mydict[
                    f"electra/encoder/layer_{i}/attention/self/value/bias"]
        getattr(
            model.encoder.layer, i
        ).attention.self.separable_conv1d.bias.data = mydict[
            f"electra/encoder/layer_{i}/attention/self/conv_attn_key/bias"].unsqueeze(
                -1)
        getattr(
            model.encoder.layer, i
        ).attention.self.separable_conv1d.depthwise.weight.data = mydict[
            f"electra/encoder/layer_{i}/attention/self/conv_attn_key/depthwise_kernel"].permute(
                1, 2, 0)
        getattr(
            model.encoder.layer, i
        ).attention.self.separable_conv1d.pointwise.weight.data = mydict[
            f"electra/encoder/layer_{i}/attention/self/conv_attn_key/pointwise_kernel"].transpose(
                0, 2)
        getattr(
            model.encoder.layer, i
        ).attention.self.conv_attn_kernel.weight.data = mydict[
            f"electra/encoder/layer_{i}/attention/self/conv_attn_kernel/kernel"].T
        getattr(
            model.encoder.layer, i
        ).attention.self.conv_attn_kernel.bias.data = mydict[
            f"electra/encoder/layer_{i}/attention/self/conv_attn_kernel/bias"]
        getattr(
            model.encoder.layer, i
        ).attention.self.conv_attn_point.weight.data = mydict[
            f"electra/encoder/layer_{i}/attention/self/conv_attn_point/kernel"].T
        getattr(
            model.encoder.layer, i
        ).attention.self.conv_attn_point.bias.data = mydict[
            f"electra/encoder/layer_{i}/attention/self/conv_attn_point/bias"]
        getattr(
            model.encoder.layer,
            i).attention.output.dense.weight.data = mydict[
                f"electra/encoder/layer_{i}/attention/output/dense/kernel"].T
        getattr(model.encoder.layer,
                i).attention.output.dense.bias.data = mydict[
                    f"electra/encoder/layer_{i}/attention/output/dense/bias"]
        getattr(
            model.encoder.layer,
            i).attention.output.LayerNorm.weight.data = mydict[
                f"electra/encoder/layer_{i}/attention/output/LayerNorm/gamma"]
        getattr(
            model.encoder.layer,
...
class ElectraSelfAttention(nn.Module):
    ...
    def forward(
        ...
        #conv_kernel_layer
        key_conv_attn_layer = self.separable_conv1d(
        ...
        context_layer = torch.cat([context_layer, conv_out], dim=2)

        context_layer = torch.reshape(context_layer, [bs, seqlen, -1])
